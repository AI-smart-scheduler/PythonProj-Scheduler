...
from datetime import datetime, time, timedelta, timezone  # Added timedelta and timezone
...
class DBService:
    ...
    # --- NEW TOKEN-SAVING READ OPERATION ---
    ...
    def get_active_context_data(self, user_id, now_dt):
        ...
        # Define the cutoff: 30 days from the client's current time
        ...
        for task in user_data.get("tasks", []):
            ...
            # Include if due soon (within 30 days) OR if it is high priority
        ...
        # Build the pruned context dictionary
        pruned_context = {
    ...
    def update_task_details(self, user_id, args):
        ...
        # Determine if it's a task or a test
        ...
        if args.get("new_name"):
            updates[f"{target_array}.$.name"] = args["new_name"]
        ...
        if args.get("new_deadline"):
            updates[f"{target_array}.$.deadline"] = args["new_deadline"]
        if args.get("new_priority"):
            updates[f"{target_array}.$.priority"] = args["new_priority"]
        if args.get("new_duration_hours") is not None:
            updates[f"{target_array}.$.duration_hours"] = args["new_duration_hours"]

        if not updates:
            return -1  # No updates requested
    ...
    def update_class_schedule(self, user_id, args):
        ...
        updates = {}
        if "new_day" in args:
            updates["schedule.$.day"] = args["new_day"]
        if "new_start_time" in args:
            updates["schedule.$.start_time"] = args["new_start_time"]
        if "new_end_time" in args:
            updates["schedule.$.end_time"] = args["new_end_time"]
    ...
    def delete_schedule_item(self, user_id, item_name):
        ...
        result_class = self.users_collection.update_one(
            {"_id": ObjectId(user_id)},
            {"$pull": {"schedule": {"subject": item_name}}}
        )
        result_task = self.users_collection.update_one(
            {"_id": ObjectId(user_id)},
            {"$pull": {"tasks": {"name": item_name}}}
        )
        result_test = self.users_collection.update_one(
            {"_id": ObjectId(user_id)},
            {"$pull": {"tests": {"name": item_name}}}
        )
        # Delete related plan blocks
        result_plan = self.users_collection.update_one(
            {"_id": ObjectId(user_id)},
    ...
    def save_study_windows(self, user_id, windows):
        """Saves the user's weekly study windows."""
        result = self.users_collection.update_one(
            {"_id": ObjectId(user_id)},
