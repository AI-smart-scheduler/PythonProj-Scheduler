...
tools = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Saves the user's ideal weekly study windows (days and times) for schedule generation.",
            "parameters": {
                "type": "object",
                "properties": {
                    "windows": {
...
def save_personalization():
    ...
    try:
        ...
        db_service.save_study_windows(user_id, windows)

        # --- DEVPRO FIX: Set onboarding_complete to True on successful save ---
        db_service.set_onboarding_complete(user_id, True)
        # --- END FIX ---

        # NOTE: Planner is still run here to allocate schedule based on new study windows
...
def chat():
    ...
    # NOTE: Daily check-in trigger is DELETED from app.py, but the frontend might still send it.
    if user_message == "trigger:daily_checkin":
        # Overriding the user message to get a generic schedule summary:
        user_message = "What do I have scheduled for today?"
